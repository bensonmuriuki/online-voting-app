...
fun ViewNomineeScreen(navController:NavHostController) {
    ...
    Column(modifier = Modifier.fillMaxSize(),
        ...
        val emptyNomineeState = remember { mutableStateOf(Nominee("","","",0,"")) }
...
fun NomineeItem(
    ...
    nomineeRepository: NomineeViewModel
) {
    ...
    Column(modifier = Modifier.fillMaxWidth() ) {
        ...
        Text(text = email)
        Text(text = post)
        Text(text = "Votes: $votes") // Displays the vote count
        Button(onClick = {
        ...
        Button(onClick = {
            nomineeRepository.voteForNominee(id)
        }) {
            Text(text = "Vote")
        }
