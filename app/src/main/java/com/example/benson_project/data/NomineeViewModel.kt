...
class NomineeViewModel(var navController: NavHostController, var context: Context) {
    ...
    var nomineeListState: MutableState<List<Nominee>> = mutableStateOf(emptyList())
    init {
    ...
    fun saveNominee(nomineeName: String, nomineeEmail: String, nomineePost: String,nomineeVotes:Int) {
        var id = System.currentTimeMillis().toString()
        var nomineeData = Nominee(nomineeName, nomineeEmail, nomineePost, nomineeVotes ,id)
    ...
    fun updateNominee(name: String, email: String, post: String,votes: Int , id: String) {
        var updateRef = FirebaseDatabase.getInstance().getReference()
            .child("nominees/$id")
        progress.show()
        var updateData = Nominee(name, email, post,votes, id)
    ...
    fun voteForNominee(nomineeId: String) {
        // Find the nominee in the list
        val nominee = nomineeListState.value.find { it.id == nomineeId }
        // Increment the vote count
        nominee?.votes = (nominee?.votes ?: 0) + 1
        // Update the UI
        nomineeListState.value = nomineeListState.value
    }
